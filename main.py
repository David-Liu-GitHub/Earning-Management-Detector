...
# Best number of neighbor is 17. Use it to build our model
...
randomForestModel = RandomForestClassifier(random_state=1, max_depth=None, min_samples_leaf=2, min_samples_split=5,
                                           n_estimators=50)
...
bestKNNClassifier = KNeighborsClassifier(n_neighbors=17, weights="distance")
...
bestRandomForestClassifier = RandomForestClassifier(max_depth=None, min_samples_leaf=2, min_samples_split=5, n_estimators=50)
...
# The second important feature is the UAA score of the evaluating year, followed by the previous year's stock price
# change. Overall, most of these factors contribute to our predictive model, and should not be removed.
...
# We saw a big improvement in the test's P value which decreased from the previous 2.5% to 0.02%. This shows that the
...
# confidence level of nearly 95% and proves that companies that restated due to SEC investigations do have, on average,
# a worse earning quality.
